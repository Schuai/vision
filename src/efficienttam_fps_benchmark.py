...
from src.realsense_ros2_publisher import (
    DEFAULT_SERIAL_NUMBER,
    enumerate_realsense_devices,
    resolve_device_serial,
)
from src.ros2_utils import binary_mask_to_bbox_xywh, bbox_xywh_to_xyxy, logits_to_binary_mask
...
def resolve_tracker_checkpoint(checkpoint_path: str | None) -> str:
    ...
    default_checkpoint = Path("checkpoints/efficienttam_s_512x512.pt").resolve()
    if default_checkpoint.is_file():
        return str(default_checkpoint)
    raise FileNotFoundError(
        "No EfficientTAM checkpoint was provided and the default "
        "`checkpoints/efficienttam_s_512x512.pt` was not found."
    )
